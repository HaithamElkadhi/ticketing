/**
 * Vercel Serverless Function — proxies /api/airtable/* → https://api.airtable.com/v0/*
 * Set env AIRTABLE_TOKEN in Vercel project settings.
 */
...
export default async function handler(req, res) {
  ...
  const host = req.headers.host || "localhost"
  const u = new URL(req.url || "/", `https://${host}`)
  const stripped = u.pathname.replace(/^\/api\/airtable\/?/, "")
  const targetUrl = `https://api.airtable.com/v0/${stripped}${u.search}`
